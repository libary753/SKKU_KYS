...
import csv
import os
import FashionNet_Landmark as fashionnet
...
class test:
    ...
    def readCsv(self,model):
        ...
        if model is 'full':
            ...
            self.visibility_list=np.vstack((arr[:,3].astype('float32'),arr[:,6].astype('float32'),arr[:,9].astype('float32'),arr[:,12].astype('float32'),arr[:,15].astype('float32'),arr[:,18].astype('float32'))).T
            self.x_list=np.vstack((arr[:,4].astype('float32'),arr[:,7].astype('float32'),arr[:,10].astype('float32'),arr[:,13].astype('float32'),arr[:,16].astype('float32'),arr[:,19].astype('float32'))).T
            self.y_list=np.vstack((arr[:,5].astype('float32'),arr[:,8].astype('float32'),arr[:,11].astype('float32'),arr[:,14].astype('float32'),arr[:,17].astype('float32'),arr[:,20].astype('float32'))).T
        
        else:
            self.visibility_list=np.vstack((arr[:,3].astype('float32'),arr[:,6].astype('float32'),arr[:,9].astype('float32'),arr[:,12].astype('float32'))).T
            self.x_list=np.vstack((arr[:,4].astype('float32'),arr[:,7].astype('float32'),arr[:,10].astype('float32'),arr[:,13].astype('float32'))).T
            self.y_list=np.vstack((arr[:,5].astype('float32'),arr[:,8].astype('float32'),arr[:,11].astype('float32'),arr[:,14].astype('float32'))).T
        
    def setOutput(self,batNum,batSize,model_type):
    ...
    def define_loss(self,fn,batSize,model_type):
    ...
    def train(self):
        model = 'upper'
        test.readCsv(model)
        fn=fashionnet.FashionNet(model)
        fn.build_net(Dropout=True)
        batsize=20
        self.define_loss(fn,batsize,model)
        ...
        fn.restore_model(sess,'C:/Users/libar/Desktop/upper/init/model') 
        print('--------------------------------------------------------------')                
                
        for j in range(1,6):
            self.readCsv('upper')
            for i in range(5049):
                self.load_batch(i,'upper',batsize)
                ...
                if i%50 is 0:
                    ...
                    if i%400 is 0:
                        ...
                        dirname = 'C:/Users/libar/Desktop/upper/'+str(i)+' epoch'
                        if not os.path.isdir(dirname):
                            os.mkdir(dirname)
                        fn.save_model(sess,'C:/Users/libar/Desktop/upper/'+str(i)+' epoch/model')

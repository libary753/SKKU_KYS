...
class FashionNet:
    ...
    def build_net(self,model_type,Dropout=False):
        ...
        if model_type is 'full':
            self.fc_3 =self.fc_layer(self.fc_2,4096,6,fcSt,fcB,'out_visibility_1',relu=False)
            self.cat_prob=tf.nn.softmax(self.fc_3)       
        elif model_type is 'upper':
            self.fc_3 =self.fc_layer(self.fc_2,4096,17,fcSt,fcB,'out_visibility_1',relu=False)
            self.cat_prob=tf.nn.softmax(self.fc_3)       
        else:
            self.fc_3 =self.fc_layer(self.fc_2,4096,12,fcSt,fcB,'out_visibility_1',relu=False)
            self.cat_prob=tf.nn.softmax(self.fc_3)       
